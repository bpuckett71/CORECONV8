...
{
    public class EquipmentTimeController : Controller
    {
        // GET: EquipmentTime
        public ActionResult Index(Token token)
        {
            TempData["Token"] = token;
            return View(new EquipmentTimeCard());
        }
        ...
        public async Task<ActionResult> ImportExcel(FormCollection formCollection)
        {
            ...
            try
            {
                ...
                string uri = ConstantValue.BaseAddress
                    + "api/EquipmentTime/Add?accountId=" + ConstantValue.account_id + "&integrationKey=" + ConstantValue.client_id + "&userTimeZone=India Standard Time";
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    return View("Index", new EquipmentTimeCard());
                }
                else
                {
        ...
        [HttpPost]
        public ActionResult JsonTimeData(EquipmentTimeCard timeCard)
        {
        ...
        public async Task<ActionResult> PostTimeCard()
        {
            ViewBag.Response = "forminput";
            Token token = TempData.Peek("Token") as Token;
            EquipmentTimeCard TimeCard = TempData.Peek("TimeCard") as EquipmentTimeCard;
            try
            {
