...
{
    public class EmployeeMiscExpensesController : Controller
    {
        // GET: EmployeeMiscExpenses
        public ActionResult Index(Token token)
        {
            TempData["Token"] = token;
            return View(new MiscExpensesTime());
        }
        ...
        public async Task<ActionResult> ImportExcel(FormCollection formCollection)
        {
            ...
            try
            {
                ...
                string uri = ConstantValue.BaseAddress
                    + "api/EmployeeMiscExpenses/Add?accountId=" + ConstantValue.account_id + "&integrationKey=" + ConstantValue.client_id + "&userTimeZone=India Standard Time";
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    return View("Index", new MiscExpensesTime());
                }
                else
                {
        ...
        [HttpPost]
        public ActionResult JsonTimeData(MiscExpensesTime timeCard)
        {
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    PropertyInfo[] infos = timeCard.GetType().GetProperties();
                    foreach (PropertyInfo info in infos)
                    {
                        dicObj.Add(info.Name, Convert.ToString(info.GetValue(timeCard, null)));
                    }
                    importExcelDto.Collection = dicObj;
                    list.Add(importExcelDto);
                    TempData["JsonData"] = Convert.ToString(JsonConvert.SerializeObject(list));
                    TempData["TimeCard"] = timeCard;
                    ViewBag.DataGenerate = "yes";
                    return View("Index", timeCard);
                }
                catch (Exception ex)
                {
                    ViewBag.Message = "Error : " + ex.Message;
        ...
        public async Task<ActionResult> PostTimeCard()
        {
            ViewBag.Response = "forminput";
            Token token = TempData.Peek("Token") as Token;
            MiscExpensesTime TimeCard = TempData.Peek("TimeCard") as MiscExpensesTime;
            try
            {
